...
test('Grouvi Nox branding is canonical with explicit legacy compatibility only',()=>{const ui=['public/index.html','public/login.html','public/setup.html','public/enroll.html','public/js/forge.js'].map(p=>fs.readFileSync(p,'utf8')).join('\n'),pkg=JSON.parse(fs.readFileSync('package.json','utf8')),manifest=JSON.parse(fs.readFileSync('RELEASE-MANIFEST.json','utf8')),build=fs.readFileSync('scripts/build-release.sh','utf8');assert.match(ui,/Grouvi Nox/);assert.match(ui,/Nox Forge/);assert.doesNotMatch(ui,/VPS Sentinel|Sentinel Forge/);assert.equal(pkg.name,'grouvi-nox-vps-sentinel');assert.equal(manifest.product,'Grouvi Nox');assert.match(build,/grouvi-nox-\$VERSION-linux/);assert.match(build,/vps-sentinel-\$VERSION-linux/);assert.ok(fs.existsSync('bin/noxctl'));const fleet=fs.readFileSync('src/fleet.js','utf8');assert.match(fleet,/X-Sentinel-Node/);assert.doesNotMatch(fleet,/X-Grouvi Nox/)});
...
test('modular CSS stays pinned to the approved refactor baseline',()=>{const dir='public/css',text=fs.readdirSync(dir).filter(x=>x.endsWith('.css')).sort().map(x=>fs.readFileSync(path.join(dir,x),'utf8')).join('');assert.equal(crypto.createHash('sha256').update(text).digest('hex'),'01e14d7043220893f57185c4b61f8ea2411b6b06a796e5fa4afc14c207750f2f')});
...
test('security UI is collector-backed and never hardcodes SSH password state',()=>{const ui=fs.readFileSync('public/app.js','utf8'),agent=fs.readFileSync('bin/privileged-collector.js','utf8'),settings=fs.readFileSync('public/js/settings-pane.js','utf8');assert.doesNotMatch(ui,/kv\('Вход по паролю',\s*'<span class="pill ok">отключён/);for(const field of ['passwordAuthentication','publicKeyAuthentication','permitRootLogin','noNewPrivileges','webInDockerGroup','configMode'])assert.ok((ui+agent+settings).includes(field),field)});
test('fleet and update UI ship clean UTF-8 copy and complete wiring',()=>{const paths=['public/index.html','public/setup.js','public/js/settings-pane.js','public/js/updates.js','bin/integration-config-broker.js'],files=paths.map(file=>fs.readFileSync(file,'utf8')).join('\n');assert.doesNotMatch(files,/\?{3,}/);assert.match(files,/id="updateOpen"/);assert.match(files,/data-pane-kind="update"/);assert.match(files,/configureFleet/);assert.match(files,/drawerFleetForm/);for(const stage of ['downloading','verifying','backup','installing','restarting','healthcheck','rollback','completed'])assert.match(files,new RegExp(stage));const lifecycle=fs.readFileSync('bin/noxctl','utf8'),updateRoute=fs.readFileSync('src/routes/api/updates.js','utf8');assert.doesNotMatch(lifecycle,/api\.github\.com\/repos\/\$repo\/releases\/latest/);assert.match(lifecycle,/github\.com\/\$repo\/releases\/latest/);assert.match(updateRoute,/const release=await freshUpdateState\(\)/);assert.doesNotMatch(updateRoute,/if\(!release\.checkedAt\)/,'the route must not serve a cache that only ever refreshes once per process');const fleetDb=fs.readFileSync('src/db/fleet.js','utf8'),updateUi=fs.readFileSync('public/js/updates.js','utf8');assert.match(fleetDb,/disk_read,disk_write/);assert.match(fleetDb,/AVG\(disk_read\)/);assert.match(updateUi,/available\?\(u\.latest/);assert.doesNotMatch(updateUi,/Full Changelog\\\*\\\*/);const html=fs.readFileSync('public/index.html','utf8'),version=JSON.parse(fs.readFileSync('package.json','utf8')).version;assert.match(html,new RegExp(`style\\.css\\?v=${version}`));assert.match(html,new RegExp(`app\\.js\\?v=${version}`));const panes=fs.readFileSync('public/js/panes.js','utf8');assert.match(panes,/['"]updatePane['"]/);});
