...
test('modular CSS stays pinned to the approved refactor baseline',()=>{const dir='public/css',text=fs.readdirSync(dir).filter(x=>x.endsWith('.css')).sort().map(x=>fs.readFileSync(path.join(dir,x),'utf8')).join('');assert.equal(crypto.createHash('sha256').update(text).digest('hex'),'2dc2d35d056e9488f28e79f435453a0a4faa9566c4ee4fb9210349ced6d69b04')});
...
test('settings rail stays paired with pane, has icons, and never dims content',()=>{const html=fs.readFileSync('public/index.html','utf8'),controller=fs.readFileSync('public/js/settings-pane.js','utf8'),css=fs.readFileSync('public/css/08-settings-drawer.css','utf8');assert.match(html,/class="btn-icon side-settings-icon"/);assert.ok((html.match(/data-settings-nav=/g)||[]).length===5);assert.ok((html.match(/settings-rail[\s\S]*?<svg/g)||[]).length>=1);assert.match(controller,/function navigate\(id\).*highlight\(id\).*content\.scrollTo/);assert.doesNotMatch(controller,/function navigate\(id\).*setRail\(false\)/);assert.match(css,/settings-rail-scrim\{display:none!important\}/);assert.match(css,/nav-scrim\{background:transparent!important\}/)});
