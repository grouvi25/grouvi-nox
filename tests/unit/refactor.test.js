...
test('settings rail stays paired with pane, has icons, and never dims content',()=>{const html=fs.readFileSync('public/index.html','utf8'),controller=fs.readFileSync('public/js/settings-pane.js','utf8'),css=fs.readFileSync('public/css/08-settings-drawer.css','utf8');assert.match(html,/class="btn-icon side-settings-icon"/);assert.ok((html.match(/data-settings-nav=/g)||[]).length===5);assert.ok((html.match(/settings-rail[\s\S]*?<svg/g)||[]).length>=1);assert.match(controller,/function navigate\(id\).*highlight\(id\)\}/);assert.doesNotMatch(controller,/function navigate\(id\).*setRail\(false\)/);assert.match(css,/settings-rail-scrim\{display:none!important\}/);assert.match(css,/nav-scrim\{background:transparent!important\}/)});
