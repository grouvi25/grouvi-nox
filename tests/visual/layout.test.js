...
try {
  ...
  await page.goto(`${base}/setup`,{waitUntil:'networkidle'});assert.equal(await page.locator('.setup-app').count(),1);await page.click('#nextBtn');await page.click('#nextBtn');assert.equal(await page.locator('#confidenceSelect').count(),1);assert.equal(await page.locator('#monitorNew').count(),1);await page.goto(`${base}/settings`,{waitUntil:'networkidle'});await page.waitForTimeout(350);assert.equal(await page.locator('#settingsPane').evaluate(x=>x.classList.contains('open')),true);assert.equal(await page.locator('.drawer-settings-section').count(),5);await page.click('#settingsClose');await page.click('#forgeOpen');await page.waitForTimeout(420);
