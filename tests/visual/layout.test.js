...
try {
  ...
  await page.goto(`${base}/setup`,{waitUntil:'networkidle'});assert.equal(await page.locator('.setup-app').count(),1);await page.click('#nextBtn');await page.click('#nextBtn');assert.equal(await page.locator('#confidenceSelect').count(),1);assert.equal(await page.locator('#monitorNew').count(),1);await page.goto(`${base}/settings`,{waitUntil:'networkidle'});await page.waitForTimeout(350);assert.equal(await page.locator('#settingsPane').evaluate(x=>x.classList.contains('open')),true);assert.equal(await page.locator('#settingsRail').evaluate(x=>x.classList.contains('open')),true);assert.equal(await page.locator('.drawer-settings-section').count(),6);await page.locator('#drawer-ai').evaluate(x=>x.scrollIntoView({block:'start'}));await page.waitForTimeout(180);assert.equal(await page.locator('[data-settings-nav="ai"]').evaluate(x=>x.classList.contains('active')),true);await page.locator('#settingsDrawerContent').evaluate(x=>x.scrollTop=x.scrollHeight);await page.waitForTimeout(180);assert.equal(await page.locator('[data-settings-nav="security"]').evaluate(x=>x.classList.contains('active')),true);await page.click('#settingsClose');await page.waitForTimeout(380);assert.equal(await page.locator('#settingsPane').evaluate(x=>x.classList.contains('open')),false);assert.equal(await page.locator('#settingsRail').evaluate(x=>x.classList.contains('open')),false);await page.click('#forgeOpen');await page.waitForTimeout(420);
