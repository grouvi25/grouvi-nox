...
try {
  ...
  await page.locator('#updateOpen').evaluate(button=>{button.hidden=false});await page.click('#updateOpen');await page.waitForTimeout(180);assert.equal(await page.locator('#updatePane').evaluate(x=>x.classList.contains('open')),true,'update drawer must open from the app bar');await page.click('#updateClose');await page.waitForTimeout(180);assert.equal(await page.locator('#updatePane').evaluate(x=>x.classList.contains('open')),false);
