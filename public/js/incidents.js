...
function renderIncidents(items,counts={},pagination={}){
  ...
  if(!items.length){$('incidentList').innerHTML='<div class="empty incident-empty">В этом фильтре всё чисто.</div>'}else{
    $('incidentList').innerHTML=items.map(i=>`<article class="incident-row" data-incident-id="${i.id}"><span class="incident-mark ${i.severity}"></span><div class="incident-main"><div class="incident-title">${esc(i.title)}</div><div class="incident-detail">${esc(i.detail||i.incident_key)}</div><div class="incident-mobile-meta">${formatWhen(i.first_seen)} · ${i.occurrences} срабатываний</div></div><div class="incident-meta">${incidentStatusPill(i.status)}<span>${formatWhen(i.first_seen)}</span><span>${i.occurrences} срабатываний</span></div><div class="incident-actions">${i.severity==='critical'&&!i.investigation?`<button class="action-sm investigate" data-incident-action="investigate" data-id="${i.id}">AI-разбор</button>`:''}${i.investigation?`<button class="action-sm" data-incident-expand="${i.id}">Разбор готов</button>`:''}<label class="incident-status-control"><span>Статус</span><select data-incident-status data-id="${i.id}" data-previous="${i.status}"><option value="open" ${i.status==='open'?'selected':''}>Открыт</option><option value="acknowledged" ${i.status==='acknowledged'?'selected':''}>Принят</option><option value="resolved" ${i.status==='resolved'?'selected':''}>Закрыт</option></select></label></div>${i.investigation?`<div class="incident-ai" id="incidentAi${i.id}" hidden><div class="incident-ai-head"><span>Nox Forge</span><time>${formatWhen(i.investigated_at)}</time></div><pre>${esc(i.investigation)}</pre></div>`:''}</article>`).join('')
  }
