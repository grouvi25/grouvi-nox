...
function renderIncidents(items,counts={},pagination={}){
  ...
  $('pageMeta').textContent=`${Math.min(page,pages)} / ${pages}`;
  $('incidentPrev').disabled=page<=1;$('incidentNext').disabled=page>=pages;
}
async function loadIncidents(status=filter,page=page){try{const offset=(page-1)*pageSize;const data=await api(`/api/incidents?status=${encodeURIComponent(status)}&limit=${pageSize}&offset=${offset}`);const pages=Math.max(1,Math.ceil(Number(data.pagination?.total||0)/pageSize));if(page>pages){page=pages;return loadIncidents(status,pages)}renderIncidents(data.incidents||[],data.counts||{},data.pagination||{})}catch(e){$('incidentList').innerHTML=`<div class="empty">Ошибка загрузки: ${esc(e.message)}</div>`}}
