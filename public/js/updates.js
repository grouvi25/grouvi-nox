...
export function createUpdateController({api,setWorkspacePane}){
  ...
  function renderRelease(){const u=release(),available=Boolean(u.available);$('updateOpen').hidden=!available;$('updateTopVersion').textContent='';$('updateCurrentVersion').textContent=`v${u.current||'?'}`;$('updateLatestVersion').textContent=`v${available?(u.latest||'?'):(u.current||'?')}`;$('updatePaneVersion').textContent=available?'доступно':'актуально';$('updatePaneVersion').className=`pill ${available?'warn':'ok'}`;$('updatePublishedAt').textContent=u.publishedAt?`Опубликовано ${new Date(u.publishedAt).toLocaleString('ru-RU')}`:'';$('updateChangelog').innerHTML=changelog(u);$('updateInstall').disabled=!available||installing||Boolean(state?.job?.running)}
