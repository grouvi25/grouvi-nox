...
export function createUpdateController({api,setWorkspacePane}){
  ...
  function renderRelease(){const u=release(),available=Boolean(u.available),failed=Boolean(u.error);$('updateOpen').hidden=!(available||failed);$('updateOpen').title=failed?'Не удалось проверить обновления':available?'Доступно обновление':'Обновления';$('updateTopVersion').textContent=available?`v${u.latest||'?'}`:'';$('updateCurrentVersion').textContent=`v${u.current||'?'}`;$('updateLatestVersion').textContent=failed?'ошибка':`v${available?(u.latest||'?'):(u.current||'?')}`;$('updatePaneVersion').textContent=failed?'ошибка':available?'доступно':'актуально';$('updatePaneVersion').className=`pill ${failed?'crit':available?'warn':'ok'}`;$('updatePublishedAt').textContent=failed?`Проверка не удалась: ${esc(u.error)}`:u.publishedAt?`Опубликовано ${new Date(u.publishedAt).toLocaleString('ru-RU')}`:'';$('updateChangelog').innerHTML=failed?`<p class="update-error">Не удалось получить состояние обновлений. Откройте панель ещё раз для повторной проверки.</p>`:changelog(u);$('updateInstall').disabled=!available||installing||Boolean(state?.job?.running)}
