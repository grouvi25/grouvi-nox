...
function fsParent(pathname) {
  ...
  function renderFleetSummary(data={}){const totals=data.totals||{};storageMode='types';distribution={roots:[],types:data.types||[]};$('fsIndexCount').textContent=`${Number(totals.files||0).toLocaleString('ru-RU')} файлов`;$('fsIndexMeta').textContent=`${formatWhen(data.at)} · агрегаты без путей`;$('fsBreadcrumb').innerHTML='<span class="crumb">sanitized / aggregate</span>';$('fsEntries').innerHTML='<div class="empty">Пути и содержимое файлов не покидают узел.</div>';$('fsSummary').innerHTML=kv('Каталоги',Number(totals.directories||0).toLocaleString('ru-RU'))+kv('Файлы',Number(totals.files||0).toLocaleString('ru-RU'))+kv('Симлинки',Number(totals.symlinks||0).toLocaleString('ru-RU'))+kv('Известный объём',bytes(Number(totals.bytes)||0));$('fsLargest').innerHTML='<div class="empty">Крупные файлы доступны только локально.</div>';$('fsRisks').innerHTML='<div class="empty">Пути риска доступны только локально.</div>';document.querySelectorAll('[data-storage-mode]').forEach(button=>button.classList.toggle('active',button.dataset.storageMode==='types'));renderStorage(true)}
  ...
  return { loadFilesystem,onEntriesClick,onEntriesKeydown,onBreadcrumbClick,up,search,setEnabled,renderFleetSummary };
}
