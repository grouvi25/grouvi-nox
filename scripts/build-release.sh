...
chmod 755 "$OUT/stage/deploy/install.sh" "$OUT/stage/bin/sentinelctl" "$OUT/stage/bin/noxctl" "$OUT/stage/bin/enroll.js" "$OUT/stage/bin/privileged-collector.js"
find "$OUT/stage/deploy" -type f -name '*.sh' -exec chmod 755 {} +

archive="$OUT/grouvi-nox-$VERSION-linux.tar.gz"
legacy="$OUT/vps-sentinel-$VERSION-linux.tar.gz"
tar --sort=name --mtime='UTC 2026-01-01' --owner=0 --group=0 --numeric-owner -C "$OUT/stage" -czf "$archive" .
cp "$archive" "$legacy"
(
  cd "$OUT"
  sha256sum "$(basename "$archive")" "$(basename "$legacy")" > SHA256SUMS
)
rm -rf "$OUT/stage"
printf 'Built %s\nLegacy compatibility %s\nSHA256 %s\n' "$archive" "$legacy" "$(sha256sum "$archive" | awk '{print $1}')"
