...
async function fastTick() {
  try {
    ...
    push('cpu', cpu.usage);
    push('mem', memory.usedPct);
    push('swap', memory.swapPct);
    push('rx', network.rxRate / 1024);
    push('tx', network.txRate / 1024);
    push('ioR', diskIo.readRate / 1024);
    push('ioW', diskIo.writeRate / 1024);
    push('load', load.one);
    ...
    console.error('[collector:fast]', e.message);
  }
}
...
async function slowTick() {
  try {
    const [filesystems, containers, pm2, systemd, fail2ban, ssh, backups, kernel] = await Promise.all([
      svc.filesystems(), svc.containers(), svc.pm2(), svc.systemd(),
      svc.fail2ban(), svc.sshActivity(), svc.backups(), proc.kernel(),
    ]);
    Object.assign(snapshot, { filesystems, containers, pm2, systemd, fail2ban, ssh, backups, kernel });
    snapshot.alerts = evaluate(snapshot);
  } catch (e) {
    console.error('[collector:slow]', e.message);
  }
}
...
async function rareTick() {
  try {
    ...
    console.error('[collector:rare]', e.message);
  }
}
...
async function certTick() {
  try {
    ...
    console.error('[collector:cert]', e.message);
  }
}
