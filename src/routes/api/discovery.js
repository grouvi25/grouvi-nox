...
async function integrationState(){try{return await integrationStatus()}catch{return{fleet:{role:config.fleet.role,nodeId:config.fleet.nodeId,nodeName:config.fleet.nodeName,hubUrl:config.fleet.hubUrl,configured:config.fleet.role!=='standalone'},telegram:{configured:Boolean(config.telegram.botToken&&config.telegram.chatId)},ai:{available:fs.existsSync(config.aiBridgeSocket),configured:fs.existsSync(config.aiBridgeSocket)},brokerUnavailable:true}}}
