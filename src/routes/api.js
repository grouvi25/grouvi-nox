...
export function createApiRouter({authMiddleware=requireAuth}={}){
  const router=express.Router();router.use(authMiddleware);router.use(coreRoutes);router.use(incidentRoutes);router.use(agentRoutes);router.use(serviceRoutes);router.use(notificationRoutes);router.use(discoveryRoutes);return router;
}
