...
clam_ok=false; clam=127; if command -v clamscan >/dev/null; then clam_ok=true; echo '=== ClamAV ===' >>"$log"; set +e; clamscan -r --infected --cross-fs=no --exclude-dir='^/var/lib/(docker|containerd)(/|$)' --exclude-dir='^/var/lib/vps-sentinel/security-scans(/|$)' "${roots[@]}" >>"$log" 2>&1; clam=$?; set -e; else missing="${missing:+$missing, }clamav"; echo '=== ClamAV is not installed, skipped ===' >>"$log"; fi
...
[[ $clam_ok == true && $rkh_ok == true ]] || result=unavailable
[[ $infected -gt 0 || $rootkits -gt 0 ]] && result=threats
finished=$(date +%s000)
ID="$id" STARTED="$started" FINISHED="$finished" RESULT="$result" CLAM="$clam" RKH="$rkh" FRESH="$fresh" INFECTED="$infected" SCANNED="$scanned" KNOWN="$known" ROOTKITS="$rootkits" WARNINGS="$warnings" CLAM_OK="$clam_ok" RKH_OK="$rkh_ok" MISSING="$missing" node - "$report" <<'NODE'
const fs=require('fs'),f=process.argv[2],n=k=>Number(process.env[k]||0),b=k=>process.env[k]==='true';const r={schema:2,id:process.env.ID,startedAt:n('STARTED'),finishedAt:n('FINISHED'),result:process.env.RESULT,missingEngines:process.env.MISSING||'',clamav:{available:b('CLAM_OK'),exitCode:n('CLAM'),infected:n('INFECTED'),scannedFiles:n('SCANNED'),knownViruses:n('KNOWN'),databaseUpdateExit:n('FRESH')},rkhunter:{available:b('RKH_OK'),exitCode:n('RKH'),possibleRootkits:n('ROOTKITS'),warnings:n('WARNINGS')}};fs.writeFileSync(f,JSON.stringify(r,null,2));fs.chmodSync(f,0o640)
NODE
